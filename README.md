...
실제 데이터셋으로 검증했습니다 — **FLEURS 한국어 40발화 × ESC-50 실제 환경 소음**,
large-v3-turbo, OpenVINO GPU ([test_audio/bench_datasets.py](test_audio/bench_datasets.py)):

| 조건 | 원본 CER | 전처리 후 | 결론 |
|---|---|---|---|
| 깨끗한 음성 | 6.1% | 6.2% | AGC 무해 |
| 조용함(×0.05) | 6.4% | 6.4% | AGC 효과 없음 (Whisper 내부 정규화가 처리) |
| 노이즈 SNR=5 | 6.9% | 7.4% | **노이즈 제거가 악화** |
| 노이즈 SNR=0 | 9.4% | 9.8% | **노이즈 제거가 악화** |

**핵심**: Whisper turbo는 자체 노이즈 강건성이 매우 뛰어나(SNR=0에서도 CER 9.4%),
노이즈 제거가 만드는 아티팩트가 득보다 실이 큽니다. Whisper-AT 연구의 경고가
실측으로 확인된 결과입니다. 따라서:

- **노이즈 제거는 기본 꺼짐**이며, 일반적으로 켜지 않기를 권합니다 (위 데이터 기준 손해).
  매우 특수한 정상 잡음 환경에서 주관적으로 도움이 될 수 있어 옵션으로만 남겨둡니다.
- **AGC는 무해**하여 기본 켜둡니다. 위 벤치마크(잘 녹음된 낭독 음성)에서는 이득이
  없었지만, 원거리·소곤거림 등 실제 마이크의 극단적 저음량은 데이터셋에 없어
  실시간 마이크 경로에서의 보정용으로 유지합니다.

> 참고: 단일 합성 TTS 클립([bench_frontend.py](test_audio/bench_frontend.py))에서는
> 노이즈 제거가 좋아 보였으나, 위 다수 실제 발화 측정에서 뒤집혔습니다 —
> 한 샘플 결과를 일반화하면 안 된다는 교훈.
